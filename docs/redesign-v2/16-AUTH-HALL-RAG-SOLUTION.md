...
账户安全页：修改密码、启用TOTP MFA、退出当前会话、退出所有会话。管理审核角色强制 MFA；普通用户自愿开启。敏感操作需近期认证；本锁定 GoTrue 版本采用已验证的第二个、独立保存的 TOTP 因子作为稳定恢复方式：唯一已验证因子不可移除，不能凭邮箱字符串解除；丢失所有因子时没有应用层绕过。微信登录不在本次必交范围；不能把 QQ 邮箱网页上的微信登录当作 DaoFlow 微信登录。
