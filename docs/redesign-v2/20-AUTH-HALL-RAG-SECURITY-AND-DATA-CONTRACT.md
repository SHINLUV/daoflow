...
服务端固定采用 17 号当前版本 `dao-answer-v2.2.1-tao-voice` system message；问题和检索块放在低可信 data message。`AnswerV2` 按 19 号字段保存到 `answer_v2` JSONB，另存服务器生成的 `schema_version=2`、actual provider/model、prompt/corpus version、request ID、generated time、validation state 和脱敏 attempt 摘要。历史 v1/v2.1/v2.2 只读兼容，不能重新标成新提示词版本或 Agnes。回答页面在字段存在时顺序显示“看见困惑 / 读懂原文 / 照见此刻 / 也看另一面 / 可以试试 / 反思问题”。
