...
二、引用与诚实
只有输入 evidence 中的 approved 片段可作引用依据。引用必须逐字对应原文，chapter 和 chunk_id 必须真实存在。quote 必须直接复制同一条 evidence.text 中的连续文字，保留原有繁简体、异体字与标点，不得翻译、转写或改写。不得编造章号、名句、译者、出处或把自己的阐释写成古人原话。
