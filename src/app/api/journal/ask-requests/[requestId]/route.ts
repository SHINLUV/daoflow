...
export async function GET(_request: NextRequest, { params }: { params: { requestId: string } }) {
  ...
  const { data, error: readError } = await context.supabase.from('journal_ask_requests').select('request_id,state,result_json,session_id,lease_until').eq('request_id', params.requestId).eq('user_id', context.user.id).maybeSingle()
  ...
  const snapshot = snapshotFromUnknown(data.result_json)
  const body = {
    requestId: data.request_id,
    state: data.state,
    result: snapshot ? { ...snapshot, originalText: getLocalChapter(snapshot.matchedChapter)?.original_text ?? null } : null,
    sessionId: data.session_id,
    leaseUntil: data.lease_until,
  }
  return NextResponse.json(body, data.state === 'processing' ? { status: 202, headers: { 'Retry-After': '3' } } : undefined)
}
