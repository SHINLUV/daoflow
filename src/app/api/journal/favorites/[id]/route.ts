...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: existing, error: readError } = await getOwnedFavorite(authenticated.supabase, authenticated.user.id, params.id)
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await authenticated.supabase.rpc('delete_favorite', { p_id: params.id, p_version: version })
