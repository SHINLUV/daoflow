...
export async function POST(request: NextRequest) {
  ...
  if (limit.kind === 'unavailable') return failure(503, 'AUTH_LIMITS_UNAVAILABLE', '账户安全限流尚未配置，暂不能注册。', id)
  ...
  if (error) return failure(503, 'AUTH_UNAVAILABLE', '注册服务暂时不可用，请稍后重试。', id)
