...
export async function POST(request: NextRequest) {
  ...
  if (before.kind === 'unavailable') return failure(503, 'AUTH_LIMITS_UNAVAILABLE', '账户安全限流尚未配置，暂不能登录。', id)
  if (before.kind === 'limited') return json({ error: { code: 'RATE_LIMITED', message: '登录尝试过多，请稍后再试。', requestId: id }, retryAfterSeconds: before.retryAfterSeconds }, 429)
  ...
  if (error) {
    ...
    if (recorded.kind === 'unavailable') return failure(503, 'AUTH_LIMITS_UNAVAILABLE', '账户安全限流未能确认，暂不能继续登录。', id)
    if (recorded.kind === 'limited') return json({ error: { code: 'RATE_LIMITED', message: '登录尝试过多，请稍后再试。', requestId: id }, retryAfterSeconds: recorded.retryAfterSeconds }, 429)
    return failure(401, 'INVALID_CREDENTIALS', '邮箱或密码不正确。', id)
  }
