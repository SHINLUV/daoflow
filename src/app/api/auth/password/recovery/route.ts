...
export async function POST(request: NextRequest) {
  ...
  if (limit.kind === 'unavailable') return clearRecoveryProof(bff, failure(503, 'AUTH_LIMITS_UNAVAILABLE', '账户安全限流尚未配置，暂不能发送重置邮件。', id), request)
  ...
  if (error) return clearRecoveryProof(bff, failure(503, 'AUTH_UNAVAILABLE', '重置邮件服务暂时不可用，请稍后重试。', id), request)
