...
export function FavoriteControls({ chapterId, originalText }: FavoriteControlsProps) {
  ...
  const loadFavorites = useCallback(async () => {
    setLoading(true)
    try {
      const response = await fetch(`/api/journal/favorites?chapterId=${chapterId}`, { cache: 'no-store' })
      if (response.ok) {
      ...
      setNotice({ kind: 'error', text: '无法连接收藏服务；原文仍可继续阅读。' })
    } finally {
      setLoading(false)
    }
  }, [chapterId])
  ...
  async function createFavorite(excerpt: string) {
    ...
    try {
      const response = await fetch('/api/journal/favorites', {
        ...
        body: JSON.stringify({ id: crypto.randomUUID(), chapterId, excerpt, note: null }),
      })
      if (!response.ok) {
        setNotice({ kind: 'error', text: (await readApiError(response)) ?? '收藏没有保存，请稍后重试。' })
        return
      }
      ...
      setNotice({ kind: 'error', text: '网络中断，未假称收藏成功；请检查连接后重试。' })
    } finally {
      setPending(false)
    }
  }
  ...
  async function saveNote(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`/api/journal/favorites/${editing.id}`, {
        method: 'PATCH',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ version: editing.version, note: note || null }),
      })
      if (!response.ok) {
        setNotice({ kind: 'error', text: (await readApiError(response)) ?? '批注没有保存，请保留输入后刷新再试。' })
        return
      }
      const { favorite } = await response.json() as { favorite: Favorite }
      ...
      setNotice({ kind: 'error', text: '网络中断，批注仍保留在输入框中。' })
    } finally {
      setPending(false)
    }
  }
  ...
  async function removeFavorite(favorite: Favorite) {
    ...
    try {
      const response = await fetch(`/api/journal/favorites/${favorite.id}`, {
        method: 'DELETE',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ version: favorite.version }),
      })
      if (!response.ok) {
        setNotice({ kind: 'error', text: (await readApiError(response)) ?? '取消收藏失败，请刷新后重试。' })
        return
      }
      ...
      setNotice({ kind: 'error', text: '网络中断，未假称已取消收藏。' })
    } finally {
      setPending(false)
    }
  }
